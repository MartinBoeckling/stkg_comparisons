...
the variable is set to false, the fine granular sub class is used from the
NLCD dataset
'
...
## Setup region area -----------------------------------------------------------
californiaBoundary <- sf::st_read('data/wildfire/californiaBoundary/CA_State_TIGER2016.shp')
californiaSP <- sf::as_Spatial(californiaBoundary)

## setup hexagonal grid --------------------------------------------------------
# define area in square meters
parquet_path <- list.files("data/wildfire/grid/h3_grid.parquet", pattern = ".parquet$", full.names=TRUE)
hexGrid <- sfarrow::st_read_parquet(parquet_path)
st_crs(hexGrid) <- prjLonLat
# Coast Line -------------------------------------------------------------------
# define bounding box
californiaBbox <- as.vector(sf::st_bbox(hexGrid))
...
californiaCoastLine <- sf::st_transform(x=californiaCoastLine, crs=prjLonLat)

# Elevation --------------------------------------------------------------------
## read data -------------------------------------------------------------------
# specify base path
basePath <- 'data/wildfire/elevation/singlefiles'
# define list of all single tif files and merge with base path
fileListElevation <- list.files(path = basePath, pattern = '.tif', full.names = TRUE)
californiaElevationFile <- stars::st_mosaic(.x=fileListElevation,
                                            dst = 'data/wildfire/elevation/californiaElevation.tif',
                                            file_ext = 'tif')
elevationRaster <- raster::raster('data/wildfire/elevation/californiaElevation.tif')
## Elevation grid -----------------------------------
...
hexGridElevation <- data.frame('ELEVATION' = elevationGridValues, "ID" = hexGrid$h3_id)
...
hexGridElevation <- hexGrid %>%
  left_join(hexGridElevation, by=join_by('h3_id' == 'ID')) %>%
  select('h3_id', 'ELEVATION.ELEVATION', 'geometry') %>%
  rename("ID" = "h3_id", "ELEVATION" = "ELEVATION.ELEVATION")
# calculate distance to shore line
# transform Spatial Polygons dataframe to simple feature object
hexGridCentroids <- sf::st_centroid(hexGrid)
hexGridCentroids <- sf::st_transform(hexGridCentroids, crs=prjLonLat)
# calculate distance of elevation Grid to california Coast Line
gridShoreDistance <- sf::st_distance(hexGrid, californiaCoastLine)
...
gridShoreDf <- data.frame(ID = hexGrid$h3_id, COASTDISTANCE = gridShoreDistance)
...
# weather ----------------------------------------------------------------------
## read data -------------------------------------------------------------------
weather <- data.table::fread('data/wildfire/weather/weather.csv')
weather <- weather %>%
...
hexGridCentroidsSf <- hexGridCentroids %>%
  st_join(hexGridElevationSf) %>%
  select(ID, ELEVATION, COASTDISTANCE, LONGITUDE, LATITUDE, geometry)
...
for (row in 1:nrow(idwDf)) {
  ...
  saveRDS(idwPredDf, paste0('data/wildfire/interpolation/idw/', interpolateColumn, '.rds'))
...
for (indicatorColumn in indicatorColumns[17:length(indicatorColumns)]) {
  ...
  saveRDS(wtPredDf, paste0('data/wildfire/interpolation/',indicatorColumn, '.rds'))
}
...
for (row in 1:nrow(krigingDf)) {
  ...
  if (rowValues$validation){
    ...
    saveRDS(validationDf, paste0('data/wildfire/interpolation/validation/', interpolateColumn, '.rds'))
  ...
  saveRDS(interpolateDf, paste0('data/wildfire/interpolation/', interpolateColumn, '.rds'))
...
hexGridSf <- hexGridSf %>%
  ...
  dplyr::select(ID, ELEVATION, COASTDISTANCE, LONGITUDE, LATITUDE, geometry)
## Landscape -------------------------------------------------------------------
# read data files
landscapeFiles <- list.files(path = 'data/wildfire/landCover', pattern = '.tif$', full.names = TRUE)
...
for (layer in 1:length(landscapeData@layers)) {
  ...
  landscapeAggDf <- landscapeAggDf %>%
    ...
    dplyr::select(-c(coverage_fraction, value)) %>%
  ...
  saveRDS(landscapeAggDf, paste0('data/wildfire/landCover/polygon/PolygonLayer',name, '.rds'))
...
# Wildfire ---------------------------------------------------------------------
wildfireDirs <- list.dirs('data/wildfire/wildfire_data', recursive = FALSE)
wildfireDf <- data.frame()
for (wildfireDir in wildfireDirs){
  year <- strsplit(wildfireDir, '/')[[1]][4]
  print(year)
...
saveRDS(wildfireDf, 'data/wildfire/wildfire_data/wildfire.rds')
saveRDS(wildfireReason, 'data/wildfire/wildfire_data/wildfireReason.rds')
...
write_parquet(weatherKrigingDf, 'data/wildfire/interpolation/pred_weather.parquet')
